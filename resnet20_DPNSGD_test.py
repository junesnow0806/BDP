...
def load_private_model(load_path, train_loader, n_epoch, epsilon, delta):
    model = resnet20()
        ...
        if epsilon == 4.0:
            for lr in learning_rates:
                for r in regularizers:
                    load_path = f"save/DPNSGD/resnet20_{epsilon}e_{lr}lr_{r}r/model.pth"
                    model = load_private_model(load_path, train_loader, n_epoch, epsilon, delta)
                    acc = test_private_model(model, test_loader)
                    
                    print(f"epsilon: {epsilon}, lr: {lr}, r: {r}, test acc: {acc}")

                    save_path = f"save/DPNSGD/resnet20_{epsilon}e_{lr}lr_{r}r/test_acc.txt"
